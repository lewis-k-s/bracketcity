...
Run this from the Entre Paréntesis repository:

```sh
npm run studio:dev
```

It starts the Studio site, saves the pulled Entre Paréntesis plugin as
...
local **All files** PHP access because the Entre Paréntesis source is a separate repository.
...
Otherwise Studio could replace the link or write into the Entre Paréntesis source tree. Run
...
This builds `dist-pages/` and an installable Entre Paréntesis ZIP in `release/`. Commit and
...
put under version control, that is the WordPress repository path. The Entre Paréntesis
repository will later move to `/Volumes/HUBSSD/code/mudlarker/nexo`. They have
separate release paths. Studio Sync manages the WordPress mirror; the Entre Paréntesis
