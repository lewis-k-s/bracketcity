...
import { createProgress, getAvailableClues, submitGuess } from "../src/engine.js";
import { esLocale } from "./fixtures.js";
...
test("directed hints can contain nested clues and reject only invalid values", () => {
  const { draft, parentId, childId } = nestedDraft();
  assert.throws(() => setReferenceDirection(draft, childId, "up"), (error) => error.code === "INVALID_DIRECTION");
  const directedParent = setReferenceDirection(draft, parentId, "left");
  assert.equal(directedParent.root[1].direction, "left");
...
test("a right prompt creates a two-sided hint, clears direction, and supports nesting", () => {
  ...
  assert.throws(
    ...
    (error) => error.code === "DUAL_HINT_DIRECTION"
  );
});
...
test("invalid author drafts cannot export", () => {
  ...
  assert.throws(() => serializeAuthorPuzzle(draft, esLocale), (error) => error.code === "INVALID_DRAFT");
});
...
test("puzzle loading preserves directions and all optional editable definition data", () => {
  ...
  definition.root[1].direction = "left";
