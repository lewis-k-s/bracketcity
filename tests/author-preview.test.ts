...
import { mapAuthorPreviewSelection, renderAuthorPreview } from "../src/author-preview.ts";
import type { AuthorPreviewDraft } from "../src/author-preview.ts";

function setup(draft: AuthorPreviewDraft): { dom: JSDOM; container: HTMLElement } {
  const dom = new JSDOM("<!doctype html><main id='preview'></main>");
  const container = dom.window.document.querySelector<HTMLElement>("#preview")!;
  renderAuthorPreview(container, draft);
  return { dom, container };
}

function select(dom: JSDOM, startNode: Node, startOffset: number, endNode: Node, endOffset: number): Selection {
  const range = dom.window.document.createRange();
  range.setStart(startNode, startOffset);
  range.setEnd(endNode, endOffset);
  const selection = dom.window.getSelection()! as unknown as Selection;
...
test("selection mapping rejects collapsed, cross-literal, bracket, and outside selections", () => {
  ...
  const collapsed = select(dom, literals.item(0).firstChild!, 1, literals.item(0).firstChild!, 1);
  assert.equal(mapAuthorPreviewSelection(collapsed, container), null);
  assert.equal(mapAuthorPreviewSelection(select(dom, literals.item(0).firstChild!, 1, literals.item(1).firstChild!, 2), container), null);
