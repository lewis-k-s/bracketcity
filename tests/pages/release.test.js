...
function browserDocument(currentScriptUrl) {
...
test("loader resolves release.js from its own URL and reports a failure", () => {
  ...
  const release = dom.window.document.head.querySelector('script[src*="release.js"]');
  assert.ok(release);
...
test("release injects hashed CSS, locale, and classic application scripts", () => {
  ...
  assert.equal(
    dom.window.document.head.querySelector("link[rel=stylesheet]").href,
    "https://owner.github.io/bracketcity/assets/nexo-def456.css"
  );
  const locale = dom.window.document.head.querySelector('script[src*="es-ES-"]');
  ...
  const app = dom.window.document.body.querySelector('script[src*="nexo-abc123.js"]');
  assert.ok(app);
