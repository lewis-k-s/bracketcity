...
import { calculateScore, compilePuzzle, createProgress, submitGuess } from "../src/engine.ts";
import {
  announce,
  createGameShell,
  getRenderedClueElement as findRenderedClueElement,
  renderPuzzle,
  showSubmitFeedback,
  updateGameSummary
} from "../src/view.ts";
...
function shell(puzzle: CompiledPuzzle, dateNavigation: DateNavigation = {}): GameView {
  return createGameShell(q("#app"), puzzle, esLocale, {
    onSubmit() {},
    onHint() {},
    onVirtualInput() {}
  }, dateNavigation);
...
function getRenderedClueElement(container: HTMLElement, clueId: string, state: string | null = null): HTMLElement {
  return findRenderedClueElement(container, clueId, state)!;
}
...
test("date selector lists catalog dates and reports a selected date change", () => {
  ...
  const select = q('[data-testid="date-selector"]');
  assert.equal(select.labels.item(0).textContent, esLocale.ui.dateSelector);
...
test("play shell hides answer-bearing titles and uses only the visible virtual keyboard", () => {
  ...
  assert.equal(q("#puzzle-title"), null);
  assert.equal(q(".puzzle-card").getAttribute("aria-label"), esLocale.ui.puzzleLabel);
  assert.equal(q(".puzzle-card").textContent.includes(branchPuzzle.title ?? ""), false);
  assert.equal(q(".instruction"), null);
  assert.equal(q(".source-note"), null);
  ...
  assert.equal(q(".keyboard-toggle"), null);
});
...
test("puzzle rendering treats answer text as text, never HTML", () => {
  ...
  const progress: Progress = { ...createProgress(puzzle), solved: { lib: "guess" }, startedAt: "2026-08-28T10:00:00Z" };
  const view = shell(puzzle);
  renderPuzzle(view.puzzleText, puzzle, progress, esLocale, () => {});
...
test("a peeked clue shows only its first letter and is no longer interactive", () => {
  installDom();
  const puzzle = compilePuzzle(branchPuzzle, esLocale);
  ...
  assert.equal(clue.dataset.hintState, "peeked");
  assert.match(clue.textContent, /l…/u);
  assert.equal(clue.getAttribute("role"), null);
  assert.doesNotMatch(clue.getAttribute("aria-label") ?? "", /Respuesta: lib/u);
});
...
test("one-sided directions render an answer slot inside the unresolved parenthesis group", () => {
  ...
  assert.equal(q('[data-answer-slot]', lib).textContent, "___");
  assert.equal(q('[data-answer-slot]', lib).getAttribute("aria-label"), "respuesta");
  assert.equal(q('[data-clue-hint="after"]', lib).textContent, branchPuzzle.clues.lib!.prompt[0]);
  assert.match(lib.textContent, /^___\s*←/u);
  const sky = getRenderedClueElement(view.puzzleText, "sky");
  assert.equal(q('[data-clue-hint="before"]', sky).textContent, branchPuzzle.clues.sky!.prompt[0]);
...
test("two-sided hints share one parenthesis group and do not expose their answer", () => {
  ...
  assert.equal(
    q('[data-clue-hint="before"]', clue).textContent,
    `${definition.clues.lib!.prompt[0]}ro`
  );
  assert.equal(q('[data-clue-hint="after"]', clue).textContent, "pista derecha");
  ...
  assert.equal(q('[data-clue-hint="before"]', clue).textContent, "libro");
  assert.equal(clue.querySelectorAll('[data-answer-slot]').length, 1);
  assert.equal(clue.outerHTML.includes(definition.clues.book!.answer), false);
});
...
test("virtual keyboard edits the real input at its current selection", () => {
  ...
  q('[data-key="ñ"]', view.keyboard!).click();
  assert.equal(view.input.value, "año");
  q('[data-key="Backspace"]', view.keyboard!).click();
...
test("completion panel contains exact final text, score, and rank", () => {
  ...
  const dateSelector = q('[data-testid="date-selector"]');
...
test("the virtual keyboard value submits through the visible form", () => {
  ...
  const view = createGameShell(q("#app"), puzzle, esLocale, {
    onSubmit(value) { submissions.push(value); },
    onHint() {},
    onVirtualInput() {}
  });
  q('[data-key="a"]', view.keyboard!).click();
