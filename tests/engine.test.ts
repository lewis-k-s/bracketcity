...
import { branchPuzzle, esLocale, freshBranch } from "./fixtures.ts";
import type { CompiledClue, CompiledPuzzle, Progress, PuzzleDefinition } from "../src/types.ts";

const ids = (clues: readonly CompiledClue[]): string[] => clues.map((clue) => clue.id);
const errorCodes = (definition: unknown): string[] => validatePuzzle(definition, esLocale).errors.map((error) => error.code);

function solve(puzzle: CompiledPuzzle, progress: Progress, ...guesses: string[]): Progress {
...
test("validator rejects invalid directions with a stable code", () => {
  const definition = freshBranch();
  definition.clues.book!.prompt[0] = { ref: "lib", direction: "up" } as any;
...
const invalidCases: ReadonlyArray<readonly [string, (definition: any) => void, string]> = [
