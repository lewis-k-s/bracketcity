...
async function copySeeds(destination) {
  await mkdir(destination, { recursive: true });
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  let result;
  try {
    result = await packagePlugin({ runChecks: !skipChecks, writeArchive: !verifyOnly });
    if (verifyOnly) {
      console.log(`Verified ${result.files.length} packaged files.`);
    } else {
      console.log(`Created ${result.archivePath} with ${result.files.length} files.`);
    }
  } finally {
